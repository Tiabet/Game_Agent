...
def build_direct_vision_prompt(request: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Synergy icon priority: a small horizontal icon strip may represent active/available synergies. The golden barrel/keg icon is the 술통 synergy and is a high-priority strategic target. If a golden barrel/keg synergy icon is visible, prefer opening/inspecting it before lower-priority icons, and record it as SYNERGY:name=술통 when confirmed or strongly indicated.\n"
        ...
        "SYNERGY:name=<name>;count=<active/required>;active_count=<active>;required_count=<required>;effect=<effect text>;required_members=<mercenary names needed to activate, comma-separated, or unknown>;members=<visible member names or unreadable_visible_icons>. "
        ...
        "RECIPE:result=<target mercenary>;grade=<legendary|mythic|unknown>;requires=<required mercenaries or materials>;slot_order=<unique,rare,normal if shown as bottom icons>;required_unique=<unique mercenary or visual slot ref>;required_rare=<rare mercenary or visual slot ref>;required_normal=<normal mercenary or visual slot ref>;source=<screen/button where seen>. "
        ...
        "On a synergy list screen, record every visible synergy name, effect, active/required count, and any readable required mercenary/member names; if only member icons are visible, use required_members=unknown and members=unreadable_visible_icons. On a legendary/mythic detail or recipe/combination screen, prioritize collecting the bottom recipe requirements: which unique, rare, and normal mercenary icons/materials are needed. If the material names are not readable, still record visual slot refs instead of dropping the recipe. If no more visible detail or scroll target remains, return back to the mercenary list. Do not tap bottom navigation from a detail screen. When a large knowledge panel is open, y>=560 bottom navigation is forbidden; return a swipe inside the panel instead.\n"
...
def build_visible_knowledge_prompt(request: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Important icon knowledge: the golden barrel/keg icon in a synergy icon strip represents the 술통 synergy, which is the highest-priority in-game synergy target. If it is visible, record a SYNERGY entry for 술통 even if the effect text is not visible yet, using unknown fields where necessary.\n"
        ...
        "If this is a synergy list, include every visible synergy row with its count, active_count, required_count, effect, and any readable required mercenary/member names. If only member icons are visible, use required_members=unknown and members=unreadable_visible_icons. If this is a legendary/mythic detail or recipe/combination screen, include the visible result and bottom recipe requirements; use required_unique, required_rare, and required_normal when the three material slots are visible.\n"
